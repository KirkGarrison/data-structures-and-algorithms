...
const sortByChildren = (chillArray) => {
  // Solution code here...
  return chillArray.sort((firstKids, addKids) => firstKids.house.localeCompare(addKids.house), {}).sort((firstKids, addKids) => firstKids.children.length - addKids.children.length, {});
};
...
const isCapitalized = (str) => {
  // Solution code here...
  const allCaps = str.match(/\b[A-Z]\w+/g);
  return allCaps !== null ? allCaps : [];
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  return arr.filter(atojCities => /^[A-J]\w+/.test(atojCities));
};
