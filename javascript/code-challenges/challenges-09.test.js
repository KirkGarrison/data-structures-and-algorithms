...
const maxInArray = (arr) => {
  // Solution code here...
  return arr.reduce((max, min) => max > min ? max : min);
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  return Object.entries(obj).map(change => change.join(': '));
};
...
const getHouses = (arr) => {
  //let houses = [];
  // Solution code here...
  return arr.map(houses => houses.house);
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const entries = arr.map(kids => Object.entries(kids));
  return entries.find(kids => kids[0][1] === character && kids.length > 3, false);
