...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    return arr.map(plusTwo => plusTwo + 2);
  }
};
...
const typeNum = (arr) => {
  // Solution code here...
  return arr.filter(filterArr => typeof filterArr === 'number');
};
...
const containsAnd = (arr) => {
  // Solution code here...
  let andArr = arr.filter(andArr => andArr.includes('and'));
  return andArr;
};
...
const oddValues = (arr) => {
  // Solution code here...
  let oddArr = arr.filter(number => number % 2);
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(otherArr => {
    if (!forbiddenValues.includes(otherArr)) return otherArr;
  });
};
